...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`env_logger`](https://crates.io/crates/env_logger).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `env_logger` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Environment-driven logger backend
...
Original work copyrighted by the upstream `env_logger` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
