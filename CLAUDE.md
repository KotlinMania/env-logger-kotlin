# Claude Code Project Instructions — env-logger-kotlin

## Project Overview

This is **env-logger-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`env_logger`](https://crates.io/crates/env_logger).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/env_logger/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/env_logger>
